...
Notice: Added soft-predictions output and minor fixes or adaptations.
"""
...
class FMOWBaseline:
    def __init__(self, params=None, argv=None):
        ...
        for arg in argv:
            ...
            if arg == '-prebuilt':
                self.params.directories['cnn_models'] = '../models'
                self.params.directories['lstm_models'] = '../models'
            if arg == '-nm':
        ...
        if self.params.use_metadata:
            ...
            self.params.files['cnn_model'] = os.path.join(self.params.directories['cnn_models'], 'cnn_image_only.model')
            self.params.files['lstm_model'] = os.path.join(self.params.directories['lstm_models'], 'lstm_image_only.model')
    ...
    def test_models(self):
        ...
        flagstr = 'with_metadata' if self.params.use_metadata else 'no_metadata'
        
        if self.params.test_cnn:
            fidCNN = open(os.path.join(self.params.directories['predictions'], 'predictions-cnn-%s.txt' % timestr), 'w')
            fidCNN2 = open(os.path.join(self.params.directories['predictions'], 'soft-predictions-cnn-%s.txt' % flagstr), 'w')
        if self.params.test_lstm:
            fidLSTM = open(os.path.join(self.params.directories['predictions'], 'predictions-lstm-%s.txt' % timestr), 'w')
            fidLSTM2 = open(os.path.join(self.params.directories['predictions'], 'soft-predictions-lstm-%s.txt' % flagstr), 'w')
